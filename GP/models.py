...
def logdet_chol(chol, name=None):
    return tf.multiply(tf.constant(2,dtype=float_type),tf.reduce_sum(tf.log(tf.linalg.diag_part(chol))), name=name)

class GPLVM(gpflow.models.BayesianGPLVM):
    def __init__(self, expectation, X_mean, X_var, Y, kern, M, Z=None, X_prior_mean=None, X_prior_var=None, train_mode='GPLVM', predict_mode='GPLVM'):
        gpflow.models.BayesianGPLVM.__init__(self, X_mean, X_var, Y, kern, M, Z, X_prior_mean, X_prior_var)
    ...
    def _GPLVM_build_likelihood(self):
        ...
        psi0 = tf.reduce_sum(self.expectation(pX, self.kern))
        psi1 = self.expectation(pX, (self.kern, self.feature))
        psi2 = tf.reduce_sum(self.expectation(pX, (self.kern, self.feature), (self.kern, self.feature)), axis=0)
        ...
        LB = tf.cholesky(B + jitter_level * tf.eye(num_inducing, dtype=float_type), name='LB')
    ...
    def _GP_build_likelihood(self):
        K = self.kern.K(self.X_mean) + tf.eye(tf.shape(self.X_mean)[0], dtype=float_type) * self.likelihood.variance
        L = tf.cholesky(K)
    ...
    def _GP_build_predict(self, Xnew, full_cov=False):
        ...
        Kmm_sigma = self.kern.K(self.X_mean) + tf.eye(tf.shape(self.X_mean)[0], dtype=float_type) * self.likelihood.variance
        Knn = self.kern.K(Xnew) if full_cov else self.kern.Kdiag(Xnew)
        f_mean, f_var = gpflow.conditionals.base_conditional(Kmn, Kmm_sigma, Knn, y, full_cov=full_cov, white=False)
    ...
    def _GPVLM_build_uncertain_predict(self, Xstarmu, Xstarvar):
        ...
        psi2star = self.expectation(pXstar, (self.kern, self.feature), (self.kern, self.feature))

        Kuu = self.kern.K(self.feature.Z) + tf.eye(num_inducing, dtype=float_type) * jitter_level # M x M
        sigma2 = self.likelihood.variance
        sigma = tf.sqrt(sigma2)
        ...
        AAT = tf.matrix_triangular_solve(L, tf.transpose(tmp), lower=True) / sigma2
        B = AAT + tf.eye(num_inducing, dtype=float_type)
        LB = tf.cholesky(B + tf.eye(num_inducing, dtype=float_type) * jitter_level, name='LB') # M x M
...
class VDMGP(gpflow.models.GPModel):
    def __init__(self, X, Y, Z, name=None):
        super(VDMGP, self).__init__(
            X,Y,None,gpflow.likelihoods.Gaussian(),None,name=name
        )
    ...
    def _build_likelihood(self):
        ...
        sigma2 = self.likelihood.variance
        Kuu = self.feature.Kuu(self.s_kern, jitter=jitter_level)

        psi0 = tf.multiply(tf.cast(n, float_type),self.sigma_f**2, name='psi0')

        q_mu_X = tf.tensordot(self.q_mu,self.X,[[1],[1]], name='q_mu_x')

        with tf.name_scope('Psi1'):
            top = tf.subtract(
                tf.reshape(q_mu_X, (K,n,1)),
                tf.reshape(tf.transpose(self.feature.Z,[1,0]),(K,1,m)),
                name='top'
            )
            bot = 1+tf.reduce_sum(
                tf.reshape(self.q_cov, (K,1,D)) * tf.reshape(self.X**2, (1,n,D))
            ,axis=2,keepdims=True)
        Psi1 = tf.exp(-(top**2)/(2*bot)) / bot**0.5
        Psi1 = tf.multiply(self.sigma_f,tf.reduce_prod(Psi1, axis=0), name='Psi1')

        with tf.name_scope('Psi2'):
            Z_diff = tf.reshape(self.feature.Z,(m,1,K))-tf.reshape(self.feature.Z,(1,m,K))
            Z_bar = 0.5*(tf.reshape(self.feature.Z,(m,1,K))+tf.reshape(self.feature.Z,(1,m,K)))

            left = self.sigma_f**2 * tf.exp(-0.25*tf.reduce_sum(Z_diff**2, axis=2))

            top = tf.reshape(q_mu_X, (K,n,1,1)) - tf.reshape(tf.transpose(Z_bar,[2,0,1]),(K,1,m,m))
            bot = tf.reshape(1+2*tf.reduce_sum(
                tf.reshape(self.q_cov, (K,1,D)) * tf.reshape(self.X**2, (1,n,D))
            ,axis=2), (K,n,1,1))
            right = tf.exp(-(top**2)/bot) / bot**0.5
            right = tf.reduce_sum(tf.reduce_prod(right, axis=0), axis=0)
        Psi2 = tf.multiply(left,right, name='Psi2')

        # Aux terms
        YY = tf.matmul(self.Y,self.Y,transpose_a=True)
        Psi1Y = tf.matmul(Psi1, self.Y, transpose_a=True)
        chol_Kuu = tf.cholesky(Kuu, name='chol_Kuu')
        Kuu_Psi2 = sigma2*Kuu + Psi2
        chol_Kuu_Psi2 = tf.cholesky(Kuu_Psi2, name='chol_Kuu_Psi2')
        #logdet_Kuu= log(|Kuu|)
        logdet_Kuu = logdet_chol(chol_Kuu, name='logdet_Kuu')
        #logdet_sigma2_Kuu_Psi2= log(|sigma2 * Kuu + Psi2|)
        logdet_sigma2_Kuu_Psi2 = logdet_chol(chol_Kuu_Psi2, name='logdet_Kuu_Psi2')
        #Kuu @ KuuiPsi2 = Psi2
        KuuiPsi2 = tf.cholesky_solve(chol_Kuu, Psi2, name='KuuiPsi2')

        #alpha= (sigma2*Kuu+Psi2).inv @ Psi1Y
        alpha = tf.matmul(Psi1, self.Y, transpose_a=True)
        alpha = tf.cholesky_solve(chol_Kuu_Psi2, alpha)
        #quadratic_form= Psi1Y.T @ alpha
        quadratic_form = tf.matmul(Psi1Y, alpha, transpose_a=True)

        F1  = -tf.cast(n, float_type)*np.log(2*np.pi) - tf.cast(n-m, float_type)*tf.log(sigma2) + logdet_Kuu
        F1 += -logdet_sigma2_Kuu_Psi2 - 1/sigma2 * YY
        F1 += 1/sigma2 * quadratic_form
        F1 += -psi0/sigma2 + tf.trace(KuuiPsi2)/sigma2

        KL = tf.reduce_sum(tf.log(self.q_cov), axis=1) - tf.cast(D,float_type)*tf.log(tf.reduce_sum(self.q_cov + self.q_mu**2, axis=1))
        KL = tf.reduce_sum(KL) + tf.cast(K*D, float_type)*tf.log(tf.cast(D,float_type))
    ...
    def _build_predict(self, Xnew, full_cov=False):
        ...
        chol_Kuu_Psi2 = tf.cholesky(Kuu_Psi2, name='chol_Kuu_Psi2')
        Psi1Y = tf.matmul(Psi1, self.Y, transpose_a=True)
        alpha = tf.cholesky_solve(chol_Kuu_Psi2, Psi1Y)

        # New Terms
        q_mu_Xnew = tf.tensordot(self.q_mu,Xnew,[[1],[1]], name='q_mu_Xnew')
        with tf.name_scope('Psi1new'):
            top = tf.subtract(
                tf.reshape(q_mu_Xnew, (K,nNew,1)),
                tf.reshape(tf.transpose(self.feature.Z,[1,0]),(K,1,m)),
                name='top'
            )
            bot = 1+tf.reduce_sum(
                tf.reshape(self.q_cov, (K,1,D)) * tf.reshape(Xnew**2, (1,nNew,D))
            ,axis=2,keepdims=True)
        Psi1new = tf.exp(-(top**2)/(2*bot)) / bot**0.5
        Psi1new = tf.multiply(self.sigma_f,tf.reduce_prod(Psi1new, axis=0), name='Psi1new')

        mean = Psi1new @ alpha

        with tf.name_scope('Psi2new'):
            Z_diff = tf.reshape(self.feature.Z,(m,1,K))-tf.reshape(self.feature.Z,(1,m,K))
            Z_bar = 0.5*(tf.reshape(self.feature.Z,(m,1,K))+tf.reshape(self.feature.Z,(1,m,K)))

            left = self.sigma_f**2 * tf.exp(-0.25*tf.reduce_sum(Z_diff**2, axis=2))

            top = tf.reshape(q_mu_Xnew, (K,nNew,1,1)) - tf.reshape(tf.transpose(Z_bar,[2,0,1]),(K,1,m,m))
            bot = tf.reshape(1+2*tf.reduce_sum(
                tf.reshape(self.q_cov, (K,1,D)) * tf.reshape(Xnew**2, (1,nNew,D))
            ,axis=2), (K,nNew,1,1))
            right = tf.exp(-(top**2)/bot) / bot**0.5
            right = tf.reduce_prod(right, axis=0)
        Psi2new = tf.multiply(left,right, name='Psi2new')

        #var= tr( (sigma2 * (sigma2*Kuu+Psi2).inv - Kuu.inv + alpha@alpha.T) @ Psi2new ) + self.sigma_f**2 + sigma2 - mean**2
        var = tf.trace(
            sigma2 * tf.cholesky_solve(tf.tile(chol_Kuu_Psi2[None], [nNew, 1, 1]), Psi2new) -
            tf.cholesky_solve(tf.tile(chol_Kuu[None], [nNew, 1, 1]), Psi2new) +
            tf.tile(tf.matmul(alpha, alpha, transpose_b=True)[None], [nNew, 1, 1]) @ Psi2new
        )
